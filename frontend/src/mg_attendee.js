...
var vm = new Vue({
  ...
  methods: {
    ...
    executeAction: function (item) {
      console.log(item.title, ' has been clicked', item.route);
      window.bridge.gotoAngularRoute(('/' + item.route))
    }
  },
...
var bridge = {
  ...
  updateMenu: function(items){
    console.log('update Menu webclient');

    // faking
    vm.menuItems = items;

    console.log('menuItems', window.vm.menuItems);
  },
...
// .factory('bridge', ['$location', '$timeout', function($location, $timeout){
//
//   bridge.gotoAngularRoute = function(path){
...
// .controller('mgAttendeeCtrl', function($scope, $http, $q, $log, $timeout){
