...
<div>
  ...
  <v-card class="ma-2" light v-for="(sf, ix) in swarfiles" :key="sf.id"
    ...
    <div v-show="sf.opened">
      <mgmt-swar-file :swarfile="swarfileopened" />
    </div>
  </v-card>
</div>
</template>
...
import MgmtSwarFile from './MgmtSwarFile';
  ...
  methods: {
      ...
      api('getSwarFiles', {id_swartrn: this.swartrn.id}).then(
        function(data) {
          ...
          data.forEach( function(v){
            v.upldate = this.formatDate(new Date(v.uploaddate));
            v.pub = pubvalues[v.status];
            v.opened = false;
            this.swarfiles.push(v);
          }.bind(this))
        }.bind(this),
        function(data){
    ...
    publish (sf) {
      api('publishSwarFile', {
        ...
        function(data) {
          console.error('failed to publish round ', data)
        }
      )
    },
    ...
    remove (sf) {
      api('removeSwarFile', {
        ...
        function(data) {
          console.error('failed to remove round ', data)
        }
      )
    },
