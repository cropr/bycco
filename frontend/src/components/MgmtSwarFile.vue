...
export default {
  ...
  methods: {
    ...
    parseSwarFile () {
      var players, six, pix, pl, lastgame;
      var swarjsonfile = JSON.parse(this.swarfile.jsonfile).Swar;
      ...
      players.forEach( function(p) {
        ...
        p.RoundArray.forEach( function(g, ix) {
          if (g.Tabel == "Absent") {
            pl.absences.push({
              round: g.RoundNr - 1
            });
          ...
          if (ix == p.RoundArray.length - 1) {  // current Round
            pix = lastgame.table;
            if (!this.pairings[pix]) this.pairings[pix] = {};
            if (lastgame.color == 'W') {
              this.pairings[pix].white = pl.name;
              this.pairings[pix].black = lastgame.opponentName;
              switch (lastgame.result.toUpperCase()) {
                case '1':
                  this.pairings[pix].result = '1-0';
                  break;
                case '½':
                  this.pairings[pix].result = '½-½';
                  break;
                case '0':
                  this.pairings[pix].result = '0-1';
                  break;
                case '1FF':
                  this.pairings[pix].result = '1-0 FF';
                  break;
                case '0FF':
                  this.pairings[pix].result = '0-1 FF';
                  break;
              }
            }
          }
