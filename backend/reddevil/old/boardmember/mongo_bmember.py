...
from ..common import conn, RdBadRequest, RdNotFound
...
class DbBoardMember:
    ...
    def create_bmember(cls, doc_in: dict ) -> str:
        ...
        try:
            conn[1][DbBoardMember.COLLECTION].insert_one(doc_in)
        except:
    ...
    def find_bmembers(cls, options: dict = {}) -> List[dict]:
        ...
        cursor = conn[1][DbBoardMember.COLLECTION].find(filter, projection=projfields)
        for doc in cursor:
    ...
    def find_bmember(cls, options: dict) -> dict:
        ...
        doc = conn[1][DbBoardMember.COLLECTION].find_one(options, projection=projfields)
        if not doc:
    ...
    def update_bmember(cls, id: str, docupdate: Dict[str, Any] ) -> dict:
        ...
        try:
            ...
            raise RdBadRequest(description="InvalidBMemberId")
        doc = conn[1][DbBoardMember.COLLECTION].find_one_and_update({'_id': oid}, 
    ...
    def remove_bmember(cls, id: str) -> None:
        ...
        try:
            ...
            raise RdBadRequest(description="InvalidBMemberId")
        rs = conn[1][DbBoardMember.COLLECTION].delete_one({'_id': oid})
