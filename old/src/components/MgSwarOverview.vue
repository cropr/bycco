...
export default {
  ...
  methods: {
    checktrns () {
      ...
      var self=this;
      _.forEach(self.swartrns, function(v, i){
        console.log('name', v.swarname, v.id, v.tournament_id);
        if (v.swarname == self.swarname) {
          self.swartrn = v;
          self.trnchoice = 'linked';
          self.swartrnid = v.id;
        }
      })
    },
    dropped (data, event) {
      ...
      reader.onload = function (evt) {
        self.jsonfile = evt.target.result;
        jsonswar = JSON.parse(self.jsonfile).Swar;
        console.log('jsonswar', jsonswar);
        self.swarname = jsonswar.TournamentDesc.Tournament;
        self.swarround = jsonswar.Player[0].RoundArray ?
          jsonswar.Player[0].RoundArray.length : 0;
        self.trnchoice = 'select';
        self.checktrns();
      };
    ...
    getTournaments () {
      ...
      api('getTournaments').then(
        function (data) {
          self.trnsa = data.trns;
          self.trns =  {};
          _.forEach(data.trns, function(v, i){
            self.trns[v.id] = v;
          });
        },
        function (data) {
    ...
    upload () {
      ...
      api('uploadSwarJson', {
        ...
        function(data){
          console.error('adding swar failed');
        }
      )
    }
  },
